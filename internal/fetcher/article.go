...
import (
	...
	"github.com/hi20160616/ms-ebc/configs"
...
func NewArticle() *Article {
	return &Article{
		WebsiteDomain: configs.Data.MS["ebc"].Domain,
		WebsiteTitle:  configs.Data.MS["ebc"].Title,
		WebsiteId:     fmt.Sprintf("%x", md5.Sum([]byte(configs.Data.MS["ebc"].Domain))),
	}
}
...
var timeout = func() time.Duration {
	t, err := time.ParseDuration(configs.Data.MS["ebc"].Timeout)
	if err != nil {
		log.Printf("[%s] timeout init error: %v",
			configs.Data.MS["ebc"].Title, err)
...
func (a *Article) fetchTitle() (string, error) {
	...
	if n == nil || len(n) == 0 {
		return "", fmt.Errorf("[%s] getTitle error, there is no element <title>",
			configs.Data.MS["ebc"].Title)
	}
	title := n[0].FirstChild.Data
	rp := strings.NewReplacer("｜東森新聞", "")
